...
const INPUT_CLASS =
  "w-full h-11 px-4 rounded-xl border border-border bg-white text-text-primary shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-blue/40 focus:border-primary-blue";
              ...
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full text-sm font-medium border transition-colors",
                      active ? "bg-soft-pink border-primary-pink text-primary-pink" : "bg-white border-border text-text-secondary hover:text-text-primary",
                    )}
                  >
                    {active && <Check className="size-3.5" aria-hidden />}
                    {category.name}
